# create-nefsc-neamap-menhaden-model-data-wchlor-a.R
######################################
# Janelle L. Morano
...
# These data are updated to integrate Aqua MODIS chlorophyll-a data
...
###################
chl <- read.csv("/Users/janellemorano/DATA/Chlorophyll-a/AquaMODIS/chlorophyll-AquaMODIS-USeastcoast-2003-2022-fall-monthly-ave.csv", header = TRUE)

library(janitor)
chl <- clean_names(chl, case = "upper_camel")
chl <- chl %>%
  mutate(across(c("Latitude", "Longitude"), round, 4))

# Lat and Lon aren't matching so ending up with NA
