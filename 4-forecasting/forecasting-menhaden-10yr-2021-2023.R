# forecasting-menhaden-10yr-2021-2023.R
######################################
# Janelle L. Morano

# Retrospective forecasting state and federal presence spring and fall data and break up into:
# Historic dataset: 2010-2020
# Forecasting dataset: 2021-2023
# GAM
# Compare
...
# Create training (historic) data, 2010-2020
...
# Create future data for predictions, 2021-2023
#-- Forecast Spring
future.spring <- data.frame()
for (i in seq(2021, 2023)) {
...
saveRDS(historic.gam.list, file = "/Users/janellemorano/Git/AtlanticMenhaden/4-forecasting/historic-GAM-results-10yr-2021-2023.rds")
saveRDS(historic.gam.summaries, file = "/Users/janellemorano/Git/AtlanticMenhaden/4-forecasting/historic-GAM-summaries-10yr-2021-2023.rds")

pa.gam.list <- readRDS("/Users/janellemorano/Git/AtlanticMenhaden/4-forecasting/historic-GAM-results-10yr-2021-2023.rds")
pa.gam.summaries <- readRDS("/Users/janellemorano/Git/AtlanticMenhaden/4-forecasting/historic-GAM-summaries-10yr-2021-2023.rds")




###########################################################################################
#----- Forecasting predictions, 2021-2023 ----------------------------------------------
###########################################################################################
...
###########################################################################################
#----- Calculate residuals between observed fit and forecast fit -----------------------
###########################################################################################
...
# Grab observed fit for 2021-2023
obsfit.sp <- obs[[1]] %>%
  filter(Year >=2021 & Year <= 2023) %>%
  arrange(Year, State, Survey)
obsfit.fa <- obs[[2]] %>%
  filter(Year >=2021 & Year <= 2023) %>%
  arrange(Year, State, Survey)
...
residuals.sp <- obsfit.sp$fit - forecast.predictions.spring.df$fit
residuals.fa <- obsfit.fa$fit - forecast.predictions.fall.df$fit

# Make database of residuals
residuals.sp.df <- cbind(obsfit.sp[,1:3], residuals.sp)
residuals.fa.df <- cbind(obsfit.fa[,1:3], residuals.fa)


# residuals
ggplot(data = residuals.sp.df, aes(x = Year, y = residuals.sp, color = State)) +
  geom_line()
...
print(combined_plot)
#ggsave(file = "/Users/janellemorano/Git/AtlanticMenhaden/4-forecasting/fig-forecast-GAM-bystate-10yr-2021-2023.png", width=7, height = 9)
...
#ggsave(file = "/Users/janellemorano/Git/AtlanticMenhaden/4-forecasting/fig-stateprop-10-yr-2021-2023.png", width=7, height = 4)
