...
# lm
model.sp <- lm(est_y ~ year, data = cog.sp)
r_squared.sp <- summary(model.sp)$r.squared
r_value.sp <- sqrt(r_squared.sp) * sign(coef(model.sp)[2])
...
c <- ggplot(data = cog.sp, aes(x = year, y = est_y)) +
  ...
  labs(x = " ", y = "UTM") +
  ggtitle("Spring") +
  annotate("text", x = min(cog.sp$year), y = max(cog.sp$est_y), 
           label = paste0("R² = ", round(r_squared.fa, 2)), hjust = 0, size = 6)


# lm
model.fa <- lm(est_y ~ year, data = cog.fa)
r_squared.fa <- summary(model.fa)$r.squared
r_value.fa <- sqrt(r_squared.fa) * sign(coef(model.fa)[2])
...
d <- ggplot(data = cog.fa, aes(x = year, y = est_y)) +
  ...
  labs(x = " ", y = "UTM") +
  ggtitle("Fall") +
  annotate("text", x = min(cog.fa$year), y = max(cog.fa$est_y), 
           label = paste0("R² = ", round(r_squared.fa, 2)), hjust = -1.2, size = 6)
...
#ggsave(file = "/Users/janellemorano/Git/menhaden-dist-ms/figure-output/Fig6A-cog.png", width=5, height = 5)
plot_grid(b, labels=c("B"))
#ggsave(file = "/Users/janellemorano/Git/menhaden-dist-ms/figure-output/Fig6B-cog.png", width=5, height = 5)
plot_grid(c, labels=c("C"))
#ggsave(file = "/Users/janellemorano/Git/menhaden-dist-ms/figure-output/Fig6C-cog.png", width=4, height = 3)
plot_grid(d, labels=c("D"))
#ggsave(file = "/Users/janellemorano/Git/menhaden-dist-ms/figure-output/Fig6D-cog.png", width=4, height = 3)

# Create a dummy legend, since I don't want the real legend to print 52 colors
# Create a dummy data frame with factor labels
...
ggplot(dummy_legend, aes(x = x, y = y, color = group)) +
  geom_point(size = 5) +  # Size adjusts how big the color blocks appear
  scale_color_manual(values = mycols) +
  theme_void() +  # Removes axes and background
  theme(legend.position = "bottom") +
  guides(color = guide_legend(title = "Groups", nrow = 1, override.aes = list(size = 10)))
ggsave(file = "/Users/janellemorano/Git/menhaden-dist-ms/figure-output/Fig6-legend-cog.png", width=6, height = 2)
