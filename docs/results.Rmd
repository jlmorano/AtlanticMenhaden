...
# Menhaden and Environmental Conditions

The total biomass of menhaden varies over the years in the spring \@ref(fig:springabun) and the fall \@ref(fig:fallabun). **Note: need to verify the units.**

```{r springabun, fig.cap= "Total biomass of Atlantic menhaden NEFSC bottom trawl surveys, 1963-2019, from the Spring survey.", echo=FALSE, message=FALSE, fig.align="center"}
library (tidyverse)
library(sp)
library (rnaturalearth)
library (rnaturalearthdata)
...
```{r fallabun, echo=FALSE, message=FALSE, fig.align="center", fig.cap= "Total biomass of Atlantic menhaden NEFSC bottom trawl surveys, 1963-2019, from Fall survey."}
...
## More to add here
temp at latitudes over time

regression between biomass and temp (maybe depth)

## correlations
Biomass is not correlated with any environmental conditions. Since water temperature has been a significant predictor of distribution among marine fishes, I will test how depth and temperature influence the spatio-temporal distribution of menhaden. Bottom temperature and sea surface temperature are highly correlated with each other, so I will use bottom temperature, also because the sampling method was with a bottom trawler.

```{r corr, echo=FALSE, message=FALSE, fig.align="center", fig.cap= "Correlation between environmental variables and biomass."}
library(GGally) #ggcorr ignores non-numerics automatically

corrdata <- read.csv("/Users/janellemorano/DATA/Atlantic_menhaden_modeling/combined-catch-envtl.csv")

ggcorr(corrdata)

# corr.spring <- corrdata %>%
#   filter(Season == 'SPRING')
# corr.fall <- corrdata %>%
#   filter(Season == 'FALL')
# 
...
![**(#fig:knots) Area the abundance is being predicted over and the number of knots used to calculate over the surface.**](/Users/janellemorano/Git/AtlanticMenhaden/docs/images/Data_and_knots.png)
...
![**(#fig:dist) Estimated spatio-temporal distribution of Atlantic menhaden, 2007-2019, in Spring. Dark blue = 0-low abundance, yellow = high abundance.**](/Users/janellemorano/Git/AtlanticMenhaden/docs/images/ln_density-predicted_spring.png)

<center>

![**(#fig:dist) Estimated spatio-temporal distribution of Atlantic menhaden, 2007-2019, in Fall. Dark blue = 0-low abundance, yellow = high abundance.**](/Users/janellemorano/Git/AtlanticMenhaden/docs/images/ln_density-predicted_fall.png)
...
![**(#fig:center) The center of gravity of the Spring population (left graph = easting, right graph = northing). Refer to Figure 5.1 for a geographic easting and northing reference.**](/Users/janellemorano/Git/AtlanticMenhaden/docs/images/center_of_gravity_spring.png){width=75%}

<center>

![**(#fig:center) The center of gravity of the Fall population (left graph = easting, right graph = northing). Refer to Figure 5.1 for a geographic easting and northing reference.**](/Users/janellemorano/Git/AtlanticMenhaden/docs/images/center_of_gravity_fall.png){width=75%}
...
![**(#fig:springrange) Latitudinal (in northing, N_km) range edge. The northern edge is BLUE (0.95 = 95% biomass) and the southern edges is RED (0.05 = 5% biomass). The centroid is GREEN (0.5 = 50% of biomass).**](/Users/janellemorano/Git/AtlanticMenhaden/docs/images/RangeEdge_N_km_spring.png){width=50%}

<center>

![**(#fig:fallrange) Latitudinal (in northing, N_km) range edge. The northern edge is BLUE (0.95 = 95% biomass) and the southern edges is RED (0.05 = 5% biomass). The centroid is GREEN (0.5 = 50% of biomass).**](/Users/janellemorano/Git/AtlanticMenhaden/docs/images/RangeEdge_N_km_fall.png){width=50%}
